...
void Map::draw(sf::RenderWindow& window, float dt)
{
    ...
    for(unsigned int y = 0; y < this->height; ++y)
    {
        for(unsigned int x = 0; x < this->width; ++x)
        {
            ...
            sf::Vector2f pos;
            pos.x = 0.5*((x - y) * this->tileSize + this->width * this->tileSize);
            pos.y = (x + y) * this->tileSize * 0.25;
            ...
            /* Draw the tile */
            this->tiles[y*this->width+x].draw(window, 0.05*dt);
        }
    }


    for(auto it : this->_interactive_obfects){
        sf::Vector2f pos, p;
        p = it->get_position();
        pos.x = 0.5*((p.x - p.y) * this->tileSize + this->width * this->tileSize);
        pos.y = (p.x + p.y) * this->tileSize * 0.25;

        it->draw_object.setPosition(pos);
        it->draw(window, dt);
    }
...
void Map::add_interactive_obfect(InteractiveObject *object)
{
    this->_interactive_obfects.push_front(object);
}
